...
class Handler {
  /**
   */
  constructor (protoStub, hookName, hook, onClean) {
  ...
  $registerMessageHandler (proxies) {
    ...
    for (let proxy of proxies) {
      ...
      session.$registerMessageHandler(this.$messageId, this)
  ...
  $unregisterMessageHandler (proxy) {
    const session = this.$protoStub._mosocket.$resolveProxy(proxy)
    session.$unregisterMessageHandler(this.$messageId)
