...
class MoSocket extends EventEmitter {
  ...
  async connect (ma) {
    ...
    this._sessions.set(session._sessionId.toString('base64'), session)
   ...
   */
  $resolveProxy (proxy) {
    const out = this._sessions.get(proxy.toString())
    if (!(out instanceof Session)) {
      throw new Error('invalid session proxy reference: ' + proxy.toString())
    }
    return out
  }
  ...
  $triggerProtocolMessage (msg) {
    if (msg.protoHash) {
      ...
      session.$triggerProtocolMessage(msg)
    } else {
